    ...
    <main className={styles.page}>
      ...
      <section className={styles.heroSection}>
        <div className={styles.pill}>
          ...
          <span>{t("apollo-course-pill")}</span>
        </div>
        <motion.h1
        ...
        <div className={styles.heroBadges}>
          ...
          <span className={styles.badge}>{APOLLO_COURSE_LESSONS.length} {t("course-step")}</span>
        </div>
      </section>
      ...
      <section className={styles.listSection}>
        <div className={styles.filterBar}>
          ...
          <span className={styles.count}>{APOLLO_COURSE_LESSONS.length} {t("course-lessons-count")}</span>
        </div>
        <ul className={styles.grid}>
          {APOLLO_COURSE_LESSONS.map((lesson, i) => (
            <motion.li
              key={lesson.id}
            ...
            >
              <Link
                href={createLocalizedPath(`/developer-section/apollo-graphql/${lesson.id}`)}
                className={styles.card}
              >
                <div className={styles.cardTop}>
                  <span
                    className={styles.difficulty}
                    style={{ background: "rgba(49, 28, 135, 0.3)", color: "#A78BFA" }}
                  >
      ...
      <div className={styles.footerActions}>
        <div style={{ display: "flex", gap: "16px", justifyContent: "center", flexWrap: "wrap" }}>
          <Link className={styles.secondaryLink} href={createLocalizedPath("/developer-section/apollo-graphql-challenges")}>
            {t("apollo-challenges-link")}
          </Link>
