...
const STORAGE_KEY_PREFIX = "apollo-challenge-attempts-";
...
export default function ApolloChallengePage() {
  ...
  const uri = `file:///apollo-challenge.tsx`;
  ...
  const REACT_UMD_URLS = [
    "https://unpkg.com/react@18/umd/react.development.js",
    "https://unpkg.com/react-dom@18/umd/react-dom.development.js",
  ];

  const buildPreviewHTML = useCallback((jsCode: string) => {
    const scriptTags = REACT_UMD_URLS.map(
      (url) => `<script crossorigin src="${url}"></script>`
    ).join("\n    ");
    ...
    return [
      ...
      "      var safeConsole = {",
      '        log: function() { var args = Array.from(arguments); parentWindow.postMessage({ type: "playground-log", message: args.map(function(a) { return typeof a === "object" ? JSON.stringify(a) : String(a); }).join(" ") }, "*"); },',
      '        warn: function() { var args = Array.from(arguments); parentWindow.postMessage({ type: "playground-log", message: "warn: " + args.join(" ") }, "*"); },',
      '        error: function() { var args = Array.from(arguments); parentWindow.postMessage({ type: "playground-log", message: "error: " + args.join(" ") }, "*"); },',
      "      };",
      "      try {",
      `        ${jsCode}`,
      ...
      '          var root = document.getElementById("root");',
      "          root.textContent = 'No default export named App was found. Console output is still captured.';",
      "        }",
      "      } catch (err) {",
      '        parentWindow.postMessage({ type: "playground-error", message: err && err.message ? err.message : String(err) }, "*");',
      "      }",
      "    </script>",
      "  </body>",
      "</html>",
    ...
    return (
      <main className={styles.page}>
        ...
        <div className={playStyles.notFound}>
          <p>{t("react-challenge-not-found") || "Challenge not found"}</p>
          <a href={createLocalizedPath("/developer-section/apollo-graphql-challenges")}>{t("apollo-challenge-back-to-list") || "Back to Apollo Challenges"}</a>
        </div>
        <Footer />
      </main>
    );
    ...
    <main className={styles.page}>
              ...
              {attemptCount >= MAX_ATTEMPTS && (
                <div style={{
                  ...
                  <span style={{ fontSize: "14px", color: "#ff9800" }}>
                    {(t("react-challenge-solution-unlocked") || "Solution unlocked after {n} attempts").replace("{n}", String(MAX_ATTEMPTS))}
                  </span>
                </div>
              )}

              <h4 className={playStyles.descSub}>{t("react-challenge-problem-code") || "Problem Code"}</h4>
              ...
              {challenge.hints.length > 0 && (
                <>
                  ...
                  >
                    ...
                    {showHints ? (t("react-challenge-hide-hints") || "Hide Hints") : (t("react-challenge-show-hints") || "Show Hints")}
                  </button>
                  {showHints && (
              ...
              {showSolution && (
                <>
                  <h4 className={playStyles.descSub} style={{ marginTop: "24px", color: "#ff9800" }}>
                    {t("react-challenge-solution") || "Solution"}
                  </h4>
          ...
          <div className={playStyles.editorColumn}>
            ...
            <div className={playStyles.toolbar}>
              <button className={playStyles.iconBtn} onClick={resetToProblem}>
                <ResetIcon fontSize="small" /> {t("react-challenge-reset") || "Reset"}
              </button>
              <button
              ...
              >
                <PlayIcon fontSize="small" /> {isRunningPreview ? (t("react-challenge-running") || "Running...") : (t("react-challenge-preview") || "Preview")}
              </button>
              <button
              ...
              >
                <CheckIcon fontSize="small" /> {t("react-challenge-submit") || "Submit"}
              </button>
            </div>
            ...
            {previewHtml && (
              <div className={playStyles.outputPanel} style={{ marginBottom: "16px" }}>
                <div className={playStyles.outputHead}>
                  {t("react-challenge-preview") || "Preview"}
                </div>
                <div style={{
                  padding: "12px",
                  background: "#0b1020",
                  ...
                  <iframe
                    ...
                    title="apollo-challenge-preview"
            ...
            <div className={playStyles.outputPanel}>
              <div className={playStyles.outputHead}>
                {t("react-challenge-result") || "Result"}
                {submitResult != null && (
                  <span className={submitResult.success ? playStyles.passLabel : playStyles.failLabel}>
                    {submitResult.passed}/{submitResult.total} {t("react-challenge-tests-passed") || "tests passed"}
                  </span>
                )}
              </div>
              <AnimatePresence mode="wait">
                {showSuccess && (
                  <motion.div
                    ...
                    exit={{ opacity: 0 }}
                  >
                    <CheckIcon className={playStyles.successIcon} /> {t("react-challenge-completed") || "Challenge Completed!"}
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              {submitResult && !submitResult.success && (
                <div className={playStyles.logList}>
                  <p style={{ color: "#ff6b6b", fontSize: "14px", marginBottom: "8px" }}>
                    {t("react-challenge-some-failed") || "Some tests failed:"}
                  </p>
              ...
              {submitResult && submitResult.success && (
                <div style={{ padding: "16px", color: "#4caf50", fontSize: "14px" }}>
                  {t("react-challenge-all-passed") || "All tests passed!"}
                </div>
              )}
              {!submitResult && !error && (
                <p className={playStyles.emptyLog}>{t("react-challenge-submit-prompt") || "Submit your code to see results"}</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div className={styles.footerActions}>
        <div className={playStyles.footerRow}>
          <a className={styles.secondaryLink} href={createLocalizedPath("/developer-section/apollo-graphql-challenges")}>
            {t("apollo-challenge-back") || "Back to Apollo Challenges"}
          </a>
