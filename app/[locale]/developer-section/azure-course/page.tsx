...
export default function AzureCourseLandingPage() {
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.heroSection}>
        ...
        >
          {t("azure-course-subtitle")}
        </motion.p>
        <div className={styles.heroBadges}>
          <span className={styles.badge}>
            <AzureIcon fontSize="small" />
            Microsoft Azure
          </span>
          <span className={styles.badge}>
          ...
          <span className={styles.badge}>{AZURE_COURSE_LESSONS.length} {t("course-step")}</span>
        </div>
      </section>
      ...
      <section className={styles.listSection}>
        <div className={styles.filterBar}>
          ...
          <span className={styles.count}>{AZURE_COURSE_LESSONS.length} {t("course-lessons-count")}</span>
        </div>
        <ul className={styles.grid}>
          {AZURE_COURSE_LESSONS.map((lesson, i) => (
            <motion.li
              key={lesson.id}
            ...
            >
              <Link
                href={createLocalizedPath(`/developer-section/azure-course/${lesson.id}`)}
                className={styles.card}
              >
                <div className={styles.cardTop}>
                  <span
                    className={styles.difficulty}
                    style={{ background: "rgba(0, 120, 212, 0.2)", color: "#0078d4" }}
                  >
